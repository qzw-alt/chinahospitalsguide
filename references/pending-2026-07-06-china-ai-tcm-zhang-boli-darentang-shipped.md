...
- **Remote-advance + clean rebase is now standard cron workflow:** the SEO/CSS team on chinahospitalguide pushed 4 commits between 07-04 and 07-06 (typography unification, Unicode character repair, thank-you.html, conversion funnel optimization). The `git fetch + git pull --rebase + git push` recipe worked cleanly in 1 call — no sitemap.xml conflict because the SEO commits touched different files (CSS/Unicode) than the cron run.
