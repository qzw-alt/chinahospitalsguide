...
- **Humanize score:** 79/100 first pass → 87/100 after removing `actually` from one H2 heading. Em-dashes: 17 (within chinahospitalguide 17-23/1200 baseline for the 2,000-2,200 word class; note the script reports raw count, not density, so this is in the correct band per the 2026-06-08 pitfall). No banned vocab in H2. No CJK contamination (`grep -P '[^\x00-\x7F]'` returned only em-dashes and en-dashes, which are correct).
